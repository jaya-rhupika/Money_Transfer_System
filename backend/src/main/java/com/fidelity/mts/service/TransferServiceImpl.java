...
public class TransferServiceImpl implements TransferService {
    ...
    public TransferResponseDto transfer(TransferRequestDto transferRequest) {
        ...
        if (validTransfer) {
            return executeTransfer(fromAccount, toAccount, amount, idempotency_key);
        } else {
            return null;
        }
    }
    ...
    public TransferResponseDto executeTransfer(Account senderAcc, Account recieverAcc, BigDecimal amountToBeDebited, String idempotency_key) {
        ...
        // ✅ Calculate points BEFORE creating the log
        int points = 0;
        ...
        // ✅ Set points before saving — only one save needed
        UUID temp = UUID.randomUUID();
        TransactionLog transactionLog = new TransactionLog(
                senderAcc.getId(), recieverAcc.getId(),
                amountToBeDebited.doubleValue(),
                TransactionStatus.SUCCESS, "NULL",
                idempotency_key, LocalDateTime.now()
        );
        transactionLog.setId(temp);
        transactionLog.setPoints(points);
        transactionLogRepository.save(transactionLog);

        // ✅ Update reward points on sender account
        if (points > 0) {
